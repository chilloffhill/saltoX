...
let locJobLevel;
let locJobLevelVSOP;
let locAnnualGrossSalary;
let locCustomPoolSize;
let locShares;
let locValuation;
let maxJobLevel;
let maxJobLevelVSOP;
...
const calcData = () => {
	...
	locCustomPoolSize = customPoolSize.value.replace(/[^0-9.]/g, '')*1;
	locShares = shares.value === '' ? 1000000 : shares.value.replace(/[^0-9.]/g, '')*1;
	locValuation;
	locJobLevel = jobLevel.value === '' ? 60 : jobLevel.value*1;
	maxJobLevel = jobLevel.value === '' ? 0.80 : jobLevel.value*1;
	locJobLevelVSOP = jobLevel.value === '' ? 60 : jobLevel.value*1;
	maxJobLevelVSOP = jobLevel.value === '' ? 0.80 : jobLevel.value*1;
	
	locAnnualGrossSalary = annualGrossSalary.value === '' ? 60000 : annualGrossSalary.value.replace(/[^0-9.]/g, '')*1;
	...
	numberOfShares = locShares * locCustomPoolSize/100;
	totalValueOfOption = ( locValuation / locShares ) * numberOfShares;
	...
	if ( typeValue === "VSOP" ) {
		...
		if ( companyOptions1 < 0 || companyOptions2 < 0 || companyOptions3 < 0 ){
			...
			salaryDifferenceSavedOp1 = locSalaryDifferenceOp1
			salaryDifferenceSavedOp2 = locSalaryDifferenceOp2
			salaryDifferenceSavedOp3 = locSalaryDifferenceOp3
...
salaryDifferenceOp1.addEventListener('change', function(event) {
    setJobLevel();
    let companyOptions2 = ( ( locAnnualGrossSalary  * locSalaryDifferenceOp2 ) / 100 * locJobLevel / 100 ) / ( totalValueOfOption / numberOfShares )
    let companyOptions1 = ( ( ( locAnnualGrossSalary + ( companyOptions2 * ( totalValueOfOption / numberOfShares ) ) / 4) - ( locAnnualGrossSalary * locSalaryDifferenceOp1 ) / 100 ) * 4 ) / ( totalValueOfOption / numberOfShares );
